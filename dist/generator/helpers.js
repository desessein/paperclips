...
exports.zipImportStatementParams = exports.mergeImportStatements = exports.generateRelationInput = exports.getRelativePath = exports.getRelationConnectInputFields = exports.getRelationScalars = exports.mapDMMFToParsedField = exports.makeImportsFromPrismaClient = exports.concatUniqueIntoArray = exports.concatIntoArray = exports.uniq = void 0;
...
const generateRelationInput = ({ field, model, allModels, templateHelpers: t, preAndSuffixClassName, canCreateAnnotation, canCreateAsPropertyAnnotation, canConnectAnnotation, canConnectOrCreateAnnotation, }) => {
    ...
    if ((0, field_classifiers_1.isAnnotatedWith)(field, canCreateAsPropertyAnnotation)) {
        const preAndPostfixedName = t.createDtoName(field.type);
        apiExtraModels.push(preAndPostfixedName);
        const modelToImportFrom = allModels.find(({ name }) => name === field.type);
        if (!modelToImportFrom)
    ...
    const classes = (0, api_decorator_1.extractAnnotation)(field, 'oneOf');
    if (classes) {
    ...
    if ((0, field_classifiers_1.isAnnotatedWith)(field, canConnectOrCreateAnnotation)) {
        const preAndPostfixedName = t.createDtoName(field.type);
        apiExtraModels.push(preAndPostfixedName);
        const modelToImportFrom = allModels.find(({ name }) => name === field.type);
        if (!modelToImportFrom) {
